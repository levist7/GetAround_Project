...
# Plot 3
st.subheader('Plot 3')
st.markdown("Distribution of rentals being on time or late by their status and checkin types")
...
# Plot 4
st.subheader('Plot 4')
st.markdown("Playing with parameters to be able to interpret previous plot a bit better.")
...
fig5 = px.box(dataset2, x='state', y='time_delta_with_previous_rental_in_minutes',
            facet_col= 'checkin_type')
...
corr_match=dataset2.corr().loc[:,['factorized_delay']].abs().sort_values(by='factorized_delay',ascending=False)
...
tot_delay_today_percent = round(100.* (tot_delay_today / tot_ended),1) #percentage

st.write("1. ", tot_delay_today," (", tot_delay_today_percent, "percent) drivers were late for the next check-in.")
...
""")
input_threshold = st.slider('3. Move the slider of delay threshold to see its impact', 0, 150, step= 5)

try : #in case user has interacted with the slider
    mask_b = dataset2 [col_] >= input_threshold
except: #in case user has not interacted with the slider yet
    mask_b = mask_a
tot_delay_tomorrow = mask_b.sum() #number of delays after introducing threshold
change_delay = tot_delay_today -tot_delay_tomorrow #number of delay issues resolved
...
type_checkin =  st.selectbox('4. Select the feature on checkin type', ["Only connect","No change"])
...
if type_checkin == "Only connect":
    st.write("\t:star:With the use of checkin type above, ", tot_delay_tomorrow_percent,  
...
fig7.update_layout(xaxis_title = 'Threshold in minutes', yaxis_title = 'Metric used to quantify the risk of losing money')
